...
To enable CORS, use the following CLI flags:
- `--cors`: Enable CORS for all origins (allows cross-origin requests)
- `--cors-allowed-domains`: A comma-separated list of domains to allow CORS from (e.g., `google.com,localhost:4157`). If this is empty and `--cors` is set, all origins will be allowed.

Example usage:
```bash
# Allow CORS from any origin
./bin/eigenda-proxy --cors

# Allow CORS only from specific domains
./bin/eigenda-proxy --cors --cors-allowed-domains="myapp.com,localhost:3000"
```
