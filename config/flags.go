...
func CLIFlags(envPrefix string, category string) []cli.Flag {
	...
	flags := []cli.Flag{
		...
		&cli.BoolFlag{
			Name:     CORSEnabledFlagName,
			Usage:    "Enable CORS for the server",
			Value:    false,
			EnvVars:  common.PrefixEnvVar(envPrefix, "CORS_ENABLED"),
			Category: category,
		},
		&cli.StringSliceFlag{
			Name:     CORSAllowedDomainsFlagName,
			Usage:    "List of domains allowed for CORS (e.g. google.com,localhost:4157). Empty list allows all origins.",
			Value:    cli.NewStringSlice(),
