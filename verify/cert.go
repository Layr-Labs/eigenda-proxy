...
import (
	...
	binding "github.com/Layr-Labs/eigenda/contracts/bindings/IEigenDACertTypeBindings"
...
type CertVerifier struct {
	log logging.Logger
	// ethConfirmationDepth is used to verify that a blob's batch commitment has been bridged to the
	// EigenDAServiceManager contract at least this many blocks in the past. To do so we make an eth_call to the
	// contract at the current block_number - ethConfirmationDepth. Hence in order to not require an archive node, this
	// value should be kept low. We force it to be < 64 (consts.EthHappyPathFinalizationDepthBlocks).
	// waitForFinalization should be used instead of ethConfirmationDepth if the user wants to wait for finality
...
func NewCertVerifier(cfg *Config, log logging.Logger) (*CertVerifier, error) {
	if cfg.EthConfirmationDepth >= uint64(consts.EthHappyPathFinalizationDepthBlocks) {
		...
		return nil, fmt.Errorf(
			"confirmation depth must be less than 64; consider using cfg.WaitForFinalization=true instead",
		)
	}
...
func (cv *CertVerifier) verifyBatchConfirmedOnChain(
	...
	// We retry up to 60 seconds (allowing for reorgs up to 5 blocks deep), but we only wait 3 seconds between each
	// retry,
	// in case (2) is the case and the node simply needs to resync, which could happen fast.
	//
	// Note that we don't verify that the batch is actually onchain at the batchMetadata's state confirmedBlockNumber,
	// because that would require an archive node. This is super unlikely if the disperser is honest, but it could
	// technically happen that a confirmed batch's block gets reorged out,
	// yet the tx is included in an earlier or later block, making the batchMetadata received from the disperser
	// no longer valid. The eigenda batcher does check for these reorgs and updates the batch's confirmation block
	// number:
	...
	if err != nil {
		return fmt.Errorf(
			"retrieving batch that was confirmed at block %v: %w",
			batchMetadata.GetConfirmationBlockNumber(),
			err,
		)
	}
...
func (cv *CertVerifier) getConfDeepBlockNumber(ctx context.Context) (*big.Int, error) {
	if cv.waitForFinalization {
		...
		// We ask for the latest finalized block. The second parameter "hydrated txs" is set to false because we don't
		// need full txs. See
		// https://github.com/ethereum/execution-apis/blob/4140e528360fea53c34a766d86a000c6c039100e/src/eth/block.yaml#L61
...
func (cv *CertVerifier) retrieveBatchMetadataHash(
	...
	if bytes.Equal(onchainHash[:], make([]byte, 32)) {
		return [32]byte{}, fmt.Errorf(
			"BatchMetadataHash not found for BatchId %d at block %d",
			batchID,
			blockNumber.Uint64(),
		)
	}
...
// They might be different in different environments (e.g. on a devnet or testnet), but they are fixed on a given
// network. We used to allow these parameters to change (via a setter function on the contract), but that then forced us
// here in the proxy
// to query for these parameters on every request, at the batch's reference block number (RBN).
// This in turn required rollup validators running this proxy to have an archive node, in case the RBN was >128 blocks
// in the past,
...
func getQuorumParametersAtLatestBlock(
	...
	if len(thresholds) > math.MaxUint8 {
		return nil, nil, fmt.Errorf(
			"thresholds received from EigenDAServiceManager contains %d > 256 quorums, which isn't possible",
			len(thresholds),
		)
	}
	...
	for _, quorum := range requiredQuorums {
		if _, ok := quorumAdversaryThresholds[quorum]; !ok {
			return nil, nil, fmt.Errorf(
				"required quorum %d does not have an adversary threshold. Was the EigenDAServiceManager properly deployed?",
				quorum,
			)
		}
	}
