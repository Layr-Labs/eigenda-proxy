	...
	var testTable = []struct {
	...
	}{
		{
			...
			requireErrorFn: func(t *testing.T, err error) {
				...
				require.Equal(t, -1, invalidCommitmentErr.StatusCode)
			},
		},
		{
			...
			requireErrorFn: func(t *testing.T, err error) {
				...
				// which returns a [verification.CertVerificationFailedError] with StatusCode 2 (inclusion proof invalid).
				// This test is brittle because it depends on the ordering of too many checks:
				// internal ordering that proxy calls the contract after RBN recency check, as well as the ordering of checks
				// in the CertVerifier contract (such that statusCode 2 is returned for inclusion proof invalid).
				// TODO: we should mock the CertVerifier contract call in the proxy, and inject a mock StatusCode which would make
