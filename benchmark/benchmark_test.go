...
func BenchmarkPutsWithSecondaryV1(b *testing.B) {
	testConfig := testutils.NewTestConfig(testutils.MemstoreBackend, false)
	putsWithSecondary(b, testConfig)
}
...
func BenchmarkPutsWithSecondaryV2(b *testing.B) {
	testConfig := testutils.NewTestConfig(testutils.MemstoreBackend, true)
	putsWithSecondary(b, testConfig)
}
